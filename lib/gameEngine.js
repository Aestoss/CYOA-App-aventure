...
async function createWorld(playerIdea) {
  ...
  const { system, user } = buildWorldCreationPrompt(playerIdea, settings.language);
  ...
  if (settings.imagesEnabled) {
    try {
      world.coverImageUrl = await generateImage({
...
// Manual edits from the world-editor form.
function updateWorld(worldId, {
  instructions, authorStyle, imageStyle, imageStylePrefix, imageStyleSuffix,
  description, objective, mature, contentWarnings
}) {
...
function addCharacter(worldId, { name, description, skills }) {
  const world = getWorld(worldId);
...
function updateCharacter(worldId, characterId, { name, description, skills }) {
...
async function playTurn(saveId, playerAction, { authorMode, authorNote } = {}) {
  ...
  const { system, user } = buildTurnPrompt({
    ...
    language: settings.language, chapterLength: settings.chapterLength, authorMode, authorNote
  });
