...
async function playTurn(worldId, playerAction) {
  ...
  const { system, user } = buildTurnPrompt({ world, memoryFacts, characters, recentTurns, playerAction, activeCharacter, trackedItems });
  const raw = await generateText({
    provider: settings.textProvider,
    system,
    user,
...
module.exports = { createWorld, playTurn, getSettings, selectCharacter, continueAfterVictory };
