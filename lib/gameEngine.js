...
const db = require('./db');
const { generateText, generateTextStream } = require('../providers/textProviders');
...
// Streaming counterpart to callText — used for the narration half of a turn
// so chapter text can be forwarded to the player as it's written. onChunk is
// optional: omitting it (e.g. for the world-background prefetch call) still
// gets the full text back, just without incremental delivery.
async function callTextStream({ system, user, kind, worldId, saveId, onChunk }) {
  const settings = getSettings();
  const { text, usage } = await generateTextStream({
    provider: settings.textProvider,
    system,
    user,
    apiKey: settings.apiKeys[settings.textProvider],
...
async function regenerateTurn(saveId, turnNumber, { action, note, onChapterChunk } = {}) {
  ...
  return playTurn(saveId, actionToUse, { authorNote: note && note.trim() ? note.trim() : undefined, onChapterChunk });
}
...
async function playTurn(saveId, playerAction, { authorMode, authorNote, onChapterChunk } = {}) {
  const save = getSave(saveId);
  ...
  // The turn is written in two calls so the prose can stream to the player
  // as it's generated (see TODO.md): first the chapter text alone, then a
  // second, non-streamed call reporting the structured state it implies.
  const promptContext = {
    world, memoryFacts, characters, recentTurns, playerAction, activeCharacter, trackedItems, secretInfo: save.secretInfo,
    language: world.language || settings.language, authorMode, authorNote
  };
  const narrationPrompt = buildNarrationPrompt({ ...promptContext, chapterLength: settings.chapterLength });
  const chapterText = await callTextStream({
    system: narrationPrompt.system, user: narrationPrompt.user,
    kind: 'turn_narration', worldId: world.id, saveId, onChunk: onChapterChunk
  });

  const statePrompt = buildStatePrompt({ ...promptContext, chapterText });
  const rawState = await callText({ system: statePrompt.system, user: statePrompt.user, kind: 'turn_state', worldId: world.id, saveId });
  const parsed = { ...parseModelJSON(rawState), chapter_text: chapterText };
