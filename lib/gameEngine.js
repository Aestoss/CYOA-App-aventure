...
function gatherTurnContext(saveId) {
  ...
  return { save, world, settings, activeCharacter, memoryFacts, characters, itemDefs, trackedItems, allTurns, recentTurns };
}
...
async function playTurn(saveId, playerAction, { authorMode, authorNote, providerOverride } = {}) {
  const { world, settings, activeCharacter, memoryFacts, characters, itemDefs, trackedItems, allTurns, recentTurns, save } = gatherTurnContext(saveId);
...
async function playTurnStreaming(saveId, playerAction, { authorMode, authorNote, providerOverride, onChapterChunk } = {}) {
  ...
  // this is cheap, and it sidesteps every edge case of a marker (start or
  // end) landing split across two chunks, which byte-by-byte patching does not.
  let rawBuffer = '';
  let visibleSent = 0;
  ...
  const forwardChapterChunk = (chunk) => {
    ...
    const visiblePortion = endIdx === -1 ? rawBuffer : rawBuffer.slice(0, endIdx);
  ...
  const statePrompt = buildStatePrompt({
    world, characters, activeCharacter, trackedItems, secretInfo: save.secretInfo, playerAction, chapterText, language
  });
