...
// db.json -- unlike chapter text, that's several hundred KB to a few MB per
...
// buildStateMasterPrompt's NEW FACTS section), but a bare string is also
...
// plot fact" rather than crash persistTurn.
...
function persistTurn({ saveId, world, playerAction, parsed, framedImagePrompt, imageUrl, itemDefs, allTurns }) {
  ...
  // they scroll out of the "recent turns" window. Each entry is normalized
  // to { fact, character, type }: character tags who a fact is specifically
  // about (null = general/world-level), type marks a fixed "biographical"
  // detail apart from still-evolving "plot" state — see gatherTurnContext
  // for how these two tags keep a fact available regardless of how many
  // turns pass, instead of relying only on the raw recent-turns window and
  // the general pool's RELEVANT_FACTS_LIMIT cap.
  const updates = parsed.state_updates || {};
  (updates.new_facts || []).forEach(rawFact => {
    const { fact, character, type } = normalizeNewFact(rawFact);
    if (!fact) return;
    db.get('memoryFacts').push({ id: uuid(), saveId, turnNumber, fact, character, type, createdAt: new Date().toISOString() }).write();
  });
...
// stored inline in db.json, no other cap on the file's growth over a long
...
function gatherTurnContext(saveId) {
  const save = getSave(saveId);
  ...
  // memoryFacts entries carry { fact, character, type } (see persistTurn /
  // normalizeNewFact). Splitting them three ways is what keeps a fact like
  // "Edgar has served 30 years" correct no matter how many turns pass
  // without anyone restating it -- confirmed missing before this: it was
  // only ever in the same flat, capped pool as fast-moving plot facts, so
  // it could be evicted by RELEVANT_FACTS_LIMIT or simply never survive
  // past the raw recent-turns window, and the model would reconstruct a
  // different, wrong number instead.
  //   - character == null && type == 'plot' -- KNOWN FACTS below: general,
  //     still-evolving state, kept as before (capped, oldest dropped first).
  //   - character == null && type == 'biographical' -- WORLD LORE: fixed
  //     setting/lore details untied to one person, uncapped.
  //   - character set -- attached directly to that character's own entry in
  //     OTHER CHARACTERS, uncapped, regardless of type.
  const allMemoryFacts = db.get('memoryFacts').filter({ saveId }).value();
  const generalFacts = allMemoryFacts.filter(f => !f.character);
  const memoryFacts = generalFacts.filter(f => f.type !== 'biographical').slice(-RELEVANT_FACTS_LIMIT);
  const worldLoreFacts = generalFacts.filter(f => f.type === 'biographical');
  const characterFactsByName = {};
  allMemoryFacts.forEach(f => {
    if (!f.character) return;
    (characterFactsByName[f.character] || (characterFactsByName[f.character] = [])).push(f.fact);
  });

  const characters = db.get('saveCharacters').filter({ saveId }).value()
    .map(c => ({ ...c, facts: characterFactsByName[c.name] || [] }));
  ...
  return { save, world, settings, activeCharacter, memoryFacts, worldLoreFacts, characters, itemDefs, trackedItems, allTurns, recentTurns };
}
...
async function playTurn(saveId, playerAction, { authorMode, authorNote, providerOverride } = {}) {
  const { world, settings, activeCharacter, memoryFacts, worldLoreFacts, characters, itemDefs, trackedItems, allTurns, recentTurns, save } = gatherTurnContext(saveId);
...
async function playTurnStreaming(saveId, playerAction, { authorMode, authorNote, providerOverride, onChapterChunk } = {}) {
  ...
  const narrationPrompt = buildNarrationPrompt({
    world, memoryFacts, worldLoreFacts, characters, recentTurns, playerAction, activeCharacter, trackedItems, secretInfo: save.secretInfo,
    language, chapterLength: settings.chapterLength, authorMode, authorNote
  });
...
async function maybeSummarize(saveId, turnNumber) {
  ...
  const { system, user } = buildSummaryPrompt(toSummarize, { characterNames, alreadyKnownFacts });
  try {
    ...
    if (summaryText) {
      db.get('memoryFacts').push({
        id: uuid(), saveId, turnNumber, fact: `(summary) ${summaryText}`, character: null, type: 'plot', createdAt: new Date().toISOString()
      }).write();
    }
    (parsed.missed_facts || []).forEach(rawFact => {
      const { fact, character, type } = normalizeNewFact(rawFact);
      if (!fact) return;
      db.get('memoryFacts').push({ id: uuid(), saveId, turnNumber, fact, character, type, createdAt: new Date().toISOString() }).write();
    });
