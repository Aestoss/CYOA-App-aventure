...
// Second, independent safety net beyond atomic writes: a rolling snapshot
// of the last-known-good file, taken periodically (not on every write --
// this file can be several MB) before it gets superseded. Atomic writes
// stop THIS corruption mechanism (truncation mid-write) for good, but a
// rollback point is still worth having for any other failure mode a JSON
// flat-file store can hit (a bad manual edit, a future bug that writes
// wrong-but-valid data, disk-level corruption) -- exactly the kind of
// thing that took hours of raw-file forensics to partially recover from
// once already, when there was no backup to fall back on at all.
...
function maybeBackup(sourcePath) {
  ...
  try {
    ...
    fs.copyFileSync(sourcePath, path.join(backupDir, `db-${now}.json`));
    const files = fs.readdirSync(backupDir).filter(f => /^db-\d+\.json$/.test(f)).sort();
    const excess = files.length - MAX_BACKUPS;
    if (excess > 0) files.slice(0, excess).forEach(f => fs.unlinkSync(path.join(backupDir, f)));
  } catch (e) {
    console.error(`[db] Periodic backup failed (non-fatal, main write still proceeds): ${e.message}`);
  }
}
