...
Skills: ${Object.entries(activeCharacter.skills || {})
  ...
  return {
    ...
    factsBlock, charactersBlock, secretInfoBlock, imageStyleBlock, recentBlock
  };
}
...
function buildNarrationPrompt({
  ...
  const b = buildTurnContextBlocks({ world, memoryFacts, characters, recentTurns, activeCharacter, trackedItems, secretInfo });
...
function buildStatePrompt({
  world, characters, activeCharacter, trackedItems, secretInfo, playerAction, chapterText, language
}) {
  const b = buildTurnContextBlocks({
    world, memoryFacts: [], characters, recentTurns: [], activeCharacter, trackedItems, secretInfo
  });
...
function splitNarrationResponse(raw) {
  ...
  const chapterPart = (markerIndex === -1 ? cleaned : cleaned.slice(0, markerIndex))
    ...
    .trim();
  if (markerIndex === -1) {
    throw new Error(`Narration response was truncated before the ${NARRATION_END_MARKER} marker — got ${cleaned.length} chars: ...${cleaned.slice(-120)}`);
  }
  ...
  try {
    ...
    if (!metaRaw.endsWith('}')) {
      throw new Error(`Narration META was truncated before valid JSON completed — got ${metaRaw.length} chars: ...${metaRaw.slice(-120)}`);
    }
    throw e;
  }
  return { chapterText: chapterPart, meta };
}
