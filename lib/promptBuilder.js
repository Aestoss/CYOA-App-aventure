...
// The Narrator's work for a turn is split into two calls so the prose can be
// streamed to the player as it's written (see TODO.md): this call writes
// ONLY the chapter text, in plain prose — no JSON, nothing to wait on before
// the first word can reach the client. buildStateMasterPrompt below then
// asks a second, non-streamed call to report back the structured game state
// (outcome, tracked items, secrets, etc.) implied by what was just written.
...
RESPONSE LANGUAGE: Write in ${languageName}.

FORMATTING: Write ${lengthRange} of narrative, second person, as short
paragraphs (2-4 sentences each), separated by a blank line, so it reads
easily on a phone screen. Give dialogue its own line rather than burying it
mid-paragraph. Respond with ONLY the chapter's prose — no title, no JSON, no
headers, no commentary before or after it.
${authorMode ? `
...
story. It isn't something the character attempted — do not run a skill check
against it.` : ''}
...
- When the player's action has a real chance of failing (a physical feat, a risky
  ...
  is given the current scene, and decide internally whether it succeeds, partially
  succeeds, or fails. A low rating against a hard attempt should fail or go badly
  more often than not; a high rating against an easy attempt should succeed
  cleanly. Narrate the outcome and its consequences in the prose — do not
  mention numbers, dice, or skill names to the player.
- For actions with no real chance of failure (looking around, talking casually,
  moving to an obvious place), there is no check to make — just narrate what happens.

VICTORY/DEFEAT:
- If VICTORY CONDITION and/or DEFEAT CONDITION are given below, judge as you
  write whether this turn's events genuinely and clearly satisfy one of them.
- Only let the story actually end once a condition is unambiguously met by
  what just happened in the fiction, not because it seems close or likely
  soon. Never end the story prematurely.
...
// The second half of a turn: given the chapter text the narration call just
// wrote, report back the structured state it implies. Never asked to
// rewrite or second-guess the prose — only to describe it.
function buildStateMasterPrompt({ language } = {}) {
  const languageName = LANGUAGE_NAMES[language] || LANGUAGE_NAMES.fr;
  return `You are the state-tracking half of the Narrator AI for a
choose-your-own-adventure game. CHAPTER TEXT YOU JUST WROTE below is the
prose you (the Narrator) just wrote for this turn. Do not rewrite it, repeat
it, or contradict it — only report the structured game state it implies.

RESPONSE LANGUAGE: Write "suggested_actions" in ${languageName}. Keep all
other JSON field names and values in English exactly as specified below.

Every response MUST be a single JSON object, with no text before or after it,
matching exactly this shape:

{
  "outcome": "success" | "partial_success" | "failure" | "n/a", // the skill-check result implied by the chapter text — see SKILL CHECKS below
  ...
  "suggested_actions": string[]  // 3 short, distinct next actions the player could take, consistent with how the chapter just ended
}

SKILL CHECKS:
- If the chapter text shows the player character attempting something with a
  real chance of failure, set "skill_used" to the single most relevant skill
  from SKILLS and "outcome" to whatever the prose actually shows happened:
  "success", "partial_success", or "failure".
- If the chapter text shows no real skill check (looking around, talking
  casually, an author-mode instruction, moving somewhere obvious...), set
  "outcome" to "n/a" and "skill_used" to null.

VICTORY/DEFEAT:
- Set "game_over" to null unless the chapter text you just read has
  unambiguously and clearly ended the story by satisfying VICTORY CONDITION
  or DEFEAT CONDITION below. Never infer an ending the prose doesn't
  actually show. If no conditions are given below, always leave it null.
- When the chapter text does show one occurring, set "game_over" to
  { "result", "text" }: "result" is "victory" or "defeat" matching which
  condition fired, and "text" is a short, satisfying closing message for the
  player that matches what just happened (you may adapt the world's own
  victory/defeat text below, or write your own in the same spirit).
...
- Only include an item in "tracked_item_updates" when the chapter text shows
  its own update instructions being genuinely satisfied — most turns will
  touch zero or one item, rarely more. Give the item's full new value (not a
  delta): e.g. for a text list, the complete updated list as one string; for
  a number, the new number.
...
- Return the FULL updated block in "secret_info" every turn (not a diff):
  ...
  add whatever the chapter text's events newly imply. Keep it compact — a
...
- "image_prompt" should describe only the scene from the chapter text you
  just read: who/what is the subject, their appearance and expression if a
  character is present, and the setting — concrete and visual, not the mood
  or art style.
...
- Everything you report must be something the chapter text actually shows — never invent an event it doesn't contain.`;
}
...
// Shared context assembly for a turn — used by both halves of the split
// Narrator call (buildNarrationPrompt and buildStatePrompt) so the two
// prompts stay in lockstep instead of drifting apart if edited separately.
function buildTurnContextBlocks({ world, memoryFacts, characters, recentTurns, activeCharacter, trackedItems, secretInfo }) {
...
Skills: ${Object.entries(activeCharacter.skills || {})
  ...
  return [worldBible, authorBlock, conditionsBlock, characterBlock, itemsBlock, factsBlock, charactersBlock, secretInfoBlock, imageStyleBlock, recentBlock].join('\n\n');
}

function buildActionLine({ playerAction, authorMode, authorNote }) {
  return `${authorNote ? `NARRATOR GUIDANCE FOR THIS RETRY (from the author, not visible to the player character — steer the outcome accordingly while still treating the line below as the player's actual action): ${authorNote}\n\n` : ''}${authorMode ? 'AUTHOR INSTRUCTION THIS TURN (out-of-character, follow directly)' : 'PLAYER ACTION THIS TURN'}: ${playerAction}`;
}

// First half of a turn: writes just the chapter's prose, streamable to the
// client as it's generated — see buildNarrationMasterPrompt above.
function buildNarrationPrompt({
  ...
  const context = buildTurnContextBlocks({ world, memoryFacts, characters, recentTurns, activeCharacter, trackedItems, secretInfo });
  const user = `${context}\n\n${buildActionLine({ playerAction, authorMode, authorNote })}`;
  return { system: buildNarrationMasterPrompt({ language, chapterLength, authorMode }), user };
}

// Second half of a turn: given the chapter text the first call just wrote,
// reports back the structured state it implies — see buildStateMasterPrompt above.
function buildStatePrompt({
  world, memoryFacts, characters, recentTurns, playerAction, activeCharacter, trackedItems, secretInfo,
  language, authorMode, authorNote, chapterText
}) {
  const context = buildTurnContextBlocks({ world, memoryFacts, characters, recentTurns, activeCharacter, trackedItems, secretInfo });
  const user = `${context}\n\n${buildActionLine({ playerAction, authorMode, authorNote })}\n\nCHAPTER TEXT YOU JUST WROTE\n${chapterText}`;
...
module.exports = {
  buildWorldCreationPrompt, buildNarrationPrompt, buildStatePrompt, buildSummaryPrompt, buildImagePrompt,
  buildCharacterGenerationPrompt, buildWorldAiEditPrompt, skillLabel
};
