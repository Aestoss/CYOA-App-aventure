...
.SYNOPSIS
  ...
  don't fit Fogbound's local image setup, to reclaim disk space.

.DESCRIPTION
  Fogbound's local image generation talks to AUTOMATIC1111's /sdapi/* API
  ...
  be used as-is:
    ...
    2. An unextracted ComfyUI portable archive (and its install script) --
       ...
       so using it would mean reworking Fogbound's image-provider code, not
...
#    for Fogbound's local image generation -- see .DESCRIPTION above).
# ---------------------------------------------------------------------------
