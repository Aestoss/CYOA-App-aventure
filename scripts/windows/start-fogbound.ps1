...
.DESCRIPTION
  ...
  What it does, in order:
    ...
    4. OLLAMA BRIDGE (final step -- this is what keeps the window open) --
       ...
       chain end to end, and pushes the result to Fogbound's Settings.
...
Write-Step "PHASE 4/4 -- Pont Ollama <-> Fogbound (Tailscale, Caddy, surveillant GPU)"
if (-not $ForgeOk) {
...
$bridgeParams = @{ FogboundUrl = $FogboundUrl; Model = $Model; SdPort = $SdPort; ChromaPort = $ChromaPort }
